...
`
	...
	terragruntConfig, err := ParseConfigString(config, mockOptionsForTest(t), DefaultTerragruntConfigPath, nil)
...
`
	...
	terragruntConfig, err := ParseConfigString(config, mockOptionsForTest(t), DefaultTerragruntJsonConfigPath, nil)
	require.NoError(t, err)
...
`
	...
	_, err := ParseConfigString(config, mockOptionsForTest(t), DefaultTerragruntConfigPath, nil)
	require.Error(t, err)
...
`, DefaultTerragruntConfigPath)
	...
	terragruntConfig, err := ParseConfigString(config, &opts, opts.TerragruntConfigPath, nil)
...
func TestParseTerragruntConfigTwoLevels(t *testing.T) {
	...
	_, actualErr := ParseConfigString(config, opts, configPath, nil)
...
func TestParseTerragruntConfigEmptyConfig(t *testing.T) {
	...
	cfg, err := ParseConfigString(config, mockOptionsForTest(t), DefaultTerragruntConfigPath, nil)
...
`
	...
	terragruntConfig, err := ParseConfigString(config, &opts, DefaultTerragruntConfigPath, nil)
	if err != nil {
		t.Fatal(err)
	}
